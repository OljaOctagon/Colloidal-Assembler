...
order_parameter::order_parameter(box* Box, particles& Particles, int MAX_coll_p_in){
	...
	void order_parameter::Calculate_Local_Order_Parameters(particles& Particles, polyhedra** N_Particle, box* Box){
		...
		for (int id=0;id<Box->N;id++){	
			Particles.Collision_List[id].Calculate_Neighbours(Particles.N_Particle[id]->cut_off);
		}
...
*/
	...
	void order_parameter::Calculate_2D_Psi(particles& Particles, box* Box){
		...
		for (int id1=0;id1<Box->N;id1++){
			for (int id2=0;id2<Box->N; id2++){
				...
				for (int pid1=0; pid1<Particles.N_Particle[id1]->N_patches;pid1++){
					for (int pid2=0; pid2<Particles.N_Particle[id1]->N_patches;pid2++){

						particle_dist_x =  Particles.N_Particle[id1]->x_patch[pid1] - Particles.N_Particle[id2]->x_patch[pid2];
						particle_dist_y =  Particles.N_Particle[id1]->y_patch[pid1] - Particles.N_Particle[id2]->y_patch[pid2];
						particle_dist_z =  Particles.N_Particle[id1]->z_patch[pid1] - Particles.N_Particle[id2]->z_patch[pid2];
						...
						patch_distance_squared = particle_dist_x*particle_dist_x + particle_dist_y*particle_dist_y + particle_dist_z*particle_dist_z;

						//ONLY if all the radii are the same width

						if ( patch_distance_squared < Particles.N_Particle[id1]->patch_cutoff_squared[pid1] && id1!= id2){

								NH_bonds = NH_bonds + 1; 

								scp_long_axis = fabs( Particles.N_Particle[id1]->long_axis.x * Particles.N_Particle[id2]->long_axis.x + 
													  Particles.N_Particle[id1]->long_axis.y * Particles.N_Particle[id2]->long_axis.y + 
		...
		Psi = ( da*double(NH_parallel) - db*double(NH_non_parallel) )/( da*double(NH_parallel) + db*double(NH_non_parallel) );	

		
	}

	void order_parameter::Calculate_Colors(int* List, int N_List, particles& Particles, box* Box){
			//Reset color List
		...
		for (int k1=0;k1<N_List;k1++){
			...
			scp_long_axis =       Particles.N_Particle[List[0]]->long_axis.x * Particles.N_Particle[id2]->long_axis.x + 
							      Particles.N_Particle[List[0]]->long_axis.y * Particles.N_Particle[id2]->long_axis.y + 
							      Particles.N_Particle[List[0]]->long_axis.z * Particles.N_Particle[id2]->long_axis.z ;
			...
			if(fabs(scp_long_axis) > scp_long_axis_TH){
				...
				scp_long_axis =   Particles.N_Particle[np_id]->long_axis.x * Particles.N_Particle[id2]->long_axis.x + 
							      Particles.N_Particle[np_id]->long_axis.y * Particles.N_Particle[id2]->long_axis.y + 
							      Particles.N_Particle[np_id]->long_axis.z * Particles.N_Particle[id2]->long_axis.z ;
	...
	void order_parameter::Calculate_2D_Psi(int* List, int N_List, particles& Particles, box* Box){
	...
	// calculates clusters and center mof mass for 2 patch particle clusters, through bonding, cluster algorithm. 
