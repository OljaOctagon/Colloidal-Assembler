...
#include "fileio.h"
	...
	void fileio::Read_Parameters(){
		...
		is_grand_canonical_ON = pt.get<string>("Monte_Carlo_parameters.Grand_Canonical");
	...
	void fileio::Save_Positions(box* Box, particles& Particles, int mc_time){
		...
		for(int id = 0; id<size; id++){
			patch_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->patch_type[0]), sizeof(Particles.N_Particle[id]->patch_type[0]));
			patch_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->patch_type[1]), sizeof(Particles.N_Particle[id]->patch_type[1]));
			patch_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->patch_type[2]), sizeof(Particles.N_Particle[id]->patch_type[2]));
			patch_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->patch_type[3]), sizeof(Particles.N_Particle[id]->patch_type[3]));	
    ...
    void fileio::Read_Patches(box* Box, particles& Particles, int mc_time){
		...
		for(int id = 0; id<size; id++){
			patch_in.read(reinterpret_cast<char*>(&Particles.N_Particle[id]->patch_type[0]), sizeof(int));
			patch_in.read(reinterpret_cast<char*>(&Particles.N_Particle[id]->patch_type[1]), sizeof(int));
			patch_in.read(reinterpret_cast<char*>(&Particles.N_Particle[id]->patch_type[2]), sizeof(int));
			patch_in.read(reinterpret_cast<char*>(&Particles.N_Particle[id]->patch_type[3]), sizeof(int));	
	...
	void fileio::Save_Orientations(box* Box, particles& Particles, int mc_time){
		...
		for(int id=0;id<Box->N;id++){
			...
			orient_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->q.x), sizeof(double));
			orient_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->q.y), sizeof(double));
			orient_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->q.z), sizeof(double));
			orient_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->q.w), sizeof(double));
			orient_out.write(reinterpret_cast<char*>(&Particles.N_Particle[id]->phi), sizeof(double));
	...
	void fileio::Read_Orientations(box* Box, particles& Particles, int mc_time){
		...
		for(int id = 0; id<Box->N; id++){
			...
			orient_in.read(reinterpret_cast<char*>(&Particles.N_Particle[id]->phi), sizeof(double));
	...
	*/
	...
	void fileio::Write_Positions_Gears(int mc_time, box* Box, particles& Particles){
	...
	void fileio::Write_NPT(int mc_time, box* Box){
	     ...
	     npt_out<<mc_time<<"     "<<Box->P_sigma<<"     "<<Box->N<<"      "<<Box->V<<"      "<<Box->packing_fraction<<endl;
