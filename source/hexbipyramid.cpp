...
void hexbipyramid::write_positions(ofstream &fout) {}

void hexbipyramid::edges_from_center() { // for hexbipyramid aligned with
                                         // coordinate ax
...
void hexbipyramid::Set_Start_Lattice_Position(int id, double box_Lx,
                                              int N_box) {
    ...
    x_center =
        Lx + l_distpx / 2.0 + double(id % N_sitesp1) * (2.0 * Lx + l_distpx);
    y_center = h_Lx + l_distpy / 2.0 +
               double((id / N_sitesp1) % N_sitesp2) * (2.0 * h_Lx + l_distpy);
    z_center =
        height_t + l_distpz / 2.0 +
        double(id / int(N_sitesp1 * N_sitesp2)) * (2.0 * height_t + l_distpz);
