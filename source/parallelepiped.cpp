#include "parallelepiped.h"
   ...
   void parallelepiped::Set_Start_Lattice_Position(int id, double box_Lx, int N_box){
		...
		x_center = vc.x/2.0 + l_distpx/2.0       + double(id%N_sitesp1)*(vc.x + l_distpx );
		y_center = vc.y/2.0 + l_distpy/2.0     + double((id/N_sitesp1)%N_sitesp2)*(vc.y + l_distpy );
		z_center = vc.z/2.0 + l_distpz/2.0 + double(id/int(N_sitesp1*N_sitesp2))*(vc.z + l_distpz );
