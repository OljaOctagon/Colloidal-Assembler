    ...
    void move::Particle_Insertion(particles& Particles, box* Box, fileio& Fileio, int mc_time){
		...
		*/
		...
		if(Particles.binary_on.compare("on")==0){
			...
			Particles.N_Particle[id]->Set_Lengths(patch_1,patch_2,patch_3,patch_4);
			Particles.N_Particle_old[id]->Set_Lengths(patch_1,patch_2,patch_3,patch_4);
...
void move::Particle_Deletion(particles& Particles, box* Box, fileio& Fileio, int mc_time){
        ...
        b_factor_pre = (double(Box->N+1)/double(Box->V))*exp(-beta*delta_U - beta*Box->mu);
		b_factor = minimum(1,b_factor_pre);
		...
		if(b_factor>=XI){
					...
					Box->packing_fraction = (Particles.N_Particle[0]->V*double(Box->N-1))/Box->V;

					

					for (int k = id+1; k<Box->N;k++){
						Particles.N_Particle[k-1]->x_center = Particles.N_Particle[k]->x_center;
						Particles.N_Particle[k-1]->y_center = Particles.N_Particle[k]->y_center;
						Particles.N_Particle[k-1]->z_center = Particles.N_Particle[k]->z_center;

						Particles.N_Particle[k-1]->phi = Particles.N_Particle[k]->phi;

						 Particles.N_Particle[k-1]->q.x = Particles.N_Particle_old[k]->q.x;
						 Particles.N_Particle[k-1]->q.y = Particles.N_Particle_old[k]->q.y;
						 Particles.N_Particle[k-1]->q.z = Particles.N_Particle_old[k]->q.z;
						 Particles.N_Particle[k-1]->q.w = Particles.N_Particle_old[k]->q.w;

						 Particles.N_Particle[k-1]->patch_type[0] = Particles.N_Particle[k]->patch_type[0];
						 Particles.N_Particle[k-1]->patch_type[1] = Particles.N_Particle[k]->patch_type[1];
						 Particles.N_Particle[k-1]->patch_type[2] = Particles.N_Particle[k]->patch_type[2];
						 Particles.N_Particle[k-1]->patch_type[3] = Particles.N_Particle[k]->patch_type[3];


						for (int l=0; l<Particles.N_Particle[k]->edge_N; l++){
							Particles.N_Particle[k-1]->x[l] = Particles.N_Particle[k]->x[l];
							Particles.N_Particle[k-1]->y[l] = Particles.N_Particle[k]->y[l];
							Particles.N_Particle[k-1]->z[l] = Particles.N_Particle[k]->z[l];
