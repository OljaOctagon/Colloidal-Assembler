		...
		do{
			 ...
			 if(collision_partner!=id){
				...
				//cout<<"collision_partner"<<collision_partner<<endl;	
			     ...
			     N_it = Calculate_Separating_Axis_GENERAL( Particles, id, collision_partner, L_axis) - 1;
				 ...
				 do{	
					 ...
					 if(L_axis_norm2>0.5){
						 ...
						 //if(Particles.N_Particle[id]->rmin < Particles.N_Particle[collision_partner]->rmin){
						//	 dL=fabs(Particles.N_Particle[collision_partner]->rmax - Particles.N_Particle[id]->rmin);
							 
						 //}
						 
						  //if(Particles.N_Particle[collision_partner]->rmin < Particles.N_Particle[id]->rmin){
							// dL=fabs(Particles.N_Particle[id]->rmax - Particles.N_Particle[collision_partner]->rmin);
						 ...
						 double a;
						 a = (Particles.N_Particle[id]->halo_cutoff/2.0) + Particles.N_Particle[id]->Lx;		 
...
double move::Calculate_Patch_Distance(int id1, int id2, int patch_id1, int patch_id2, particles& Particles, box* Box){
	...
	particle_dist_x =  Particles.N_Particle[id1]->x_patch[patch_id1] - Particles.N_Particle[id2]->x_patch[patch_id2];
	particle_dist_y =  Particles.N_Particle[id1]->y_patch[patch_id1] - Particles.N_Particle[id2]->y_patch[patch_id2];
	particle_dist_z =  Particles.N_Particle[id1]->z_patch[patch_id1] - Particles.N_Particle[id2]->z_patch[patch_id2];
	...
	particle_dist_sq = particle_dist_x*particle_dist_x + particle_dist_y*particle_dist_y + particle_dist_z*particle_dist_z;
	...
	for (int id1=0;id1<Box->N;id1++){
		for (int id2=0;id2<Box->N; id2++){
			...
			for (int pid1=0; pid1<Particles.N_Particle[id1]->N_patches;pid1++){
				...
				for (int pid2=0; pid2<Particles.N_Particle[id2]->N_patches;pid2++){

					patch_distance_squared = Calculate_Patch_Distance(id1, id2, pid1, pid2, Particles, Box);
					...
					if ( patch_distance_squared < Particles.N_Particle[id1]->patch_cutoff_squared[pid1] && id1!= id2){
						...
						patch_energy_ij = Particles.N_Particle[id1]->patch_energy[l][m];
