...
void pmove::Rot_Cluster_Move(particles &Particles, box *Box, fileio &Fileio,
    ...
    for(int k=0;k<Particles.Res_Size;k++){
            for(int j=0;j<Particles.N_Particle[k]->edge_N;j++){
                    xyz_out<<"cb"<<"       "<<Particles.N_Particle[k]->x[j]<<"
    "<<Particles.N_Particle[k]->y[j]<<" "<<Particles.N_Particle[k]->z[j]<<endl;
...
int mc_time) {
        ...
        do {
                 Particles.Collision_List[List[k]].Calculate_OP(Box, List[k],
Particles.N_Particle, Particles.N_Particle[0]->cut_off, Particles.MAX_coll_p);
                 //Collision Test
                 Collision_Test( Particles, Box, List[k],
Particles.Collision_List); k++;
        ...
        if (exit_status == 0){
                ...
                for (int k=0;k<N_List;k++){
                        ...
                        for(int
cp1=0;cp1<Particles.Collision_List[m].Nm;cp1++){ overlapped=false; list_j =
Particles.Collision_List[m].Elements[cp1].nl_id;

                                for(int cp2=0;
cp2<Particles.Collision_List_old[m].Nm; cp2++){ list_k =
Particles.Collision_List_old[m].Elements[cp2].nl_id;
                                ...
                                if (overlapped==false){
                                        ...
                                        e_new = Calculate_Potential(m,list_j,
Particles, Box); b_factor = b_factor*exp(-beta*e_new);
                ...
                // Calculate factors for particels that were interacting in the
last time step
                ...
                for (int k=0;k<N_List;k++){
                        ...
                        for(int
cp2=0;cp2<Particles.Collision_List_old[m].Nm;cp2++){ overlapped = false; list_k
= Particles.Collision_List_old[m].Elements[cp2].nl_id; for(int cp1=0;
cp1<Particles.Collision_List[m].Nm; cp1++){ list_j =
Particles.Collision_List[m].Elements[cp1].nl_id;
                                ...
                                if (overlapped==false){
                                        ...
                                        e_old = Calculate_Potential(m,list_k,
Particles, Box); b_factor = b_factor*exp(beta*e_old);


                                        //move back

                                        Trans_Update_Positions(Particles, m,
trans_vec); Particles.Check_Periodic_CM(m, Box);
                                        Update_Periodic_Positions(Particles,
Box, m); Particles.N_Particle[m]->Calculate_Axis();
                                        Particles.N_Particle[m]->Calculate_Patch_Position();
                ...
                ////////cout<<"Boltzmann factor for newly and oldy bonded
particles "<<b_factor<<endl;
                ...
                //////cout<<"Boltzmann factor for Pseudocluster
"<<p_factor<<endl;
                ...
                //////cout<<"Boltzmann factor failed internal links
"<<s_factor<<endl;
                ...
                if (b_factor<XI){
                        ...
                        //////////cout<<"Number of not moved particles
"<<N_List<<endl; for (int k=0;k<N_List;k++){ Reset_Positions(Particles,
List[k]); Particles.N_Particle[List[k]]->Calculate_Axis();
                                Particles.N_Particle[List[k]]->Calculate_Patch_Position();
