#include "particles.h"
    ...
    void particles::Make_Cell_List(box* Box){
		...
		N_c_float = double(N_c); 
		 ...
		 for(int id = 0; id<number_of_cells; id++){
			 ...
			 Cell[id].x_center = Box->x[0] + Cell[0].Lx/2.0 + Cell[0].Lx*double(id%N_c);
			 Cell[id].y_center = Box->y[0] + Cell[0].Ly/2.0 + Cell[0].Ly*double((id/N_c)%N_c);	
			 Cell[id].z_center = Box->z[0] + Cell[0].Lz/2.0 + Cell[0].Lz*double(id/(N_c*N_c));				
		 ...
		 //Create Cell Lists: assign to each cell all particles in it, Cell_List[c_id][0] gives the number of particles in cell
			 ...
			 for( int n_x = 0; n_x<3;n_x++){
			     ...
			     for(int n_y = 0; n_y<3; n_y++){
					 ...
					 for(int n_z = 0; n_z<3; n_z++){
						 ...
						 Cell[c_id].Neighbour[n_x][n_y][n_z] = Cell_nx + Cell_ny*N_c + Cell_nz*N_c*N_c;	
	...
	void collision_list::Calculate(box* Box, int id, int* Id_Cell_List, int** Cell_List, cell* Cell, polyhedra** N_Particle, double Cut_Off, int MAX_coll_p){
	   ...
	   void collision_list::Calculate_OP(box* Box, int id, polyhedra** N_Particle, double Cut_Off, int MAX_coll_p){
		...
		void collision_list::Calculate_Bonds(box* Box, int id1, polyhedra** N_Particle, int MAX_coll_p){
        ...
        for (int pid1=0; pid1<N_Particle[id1]->N_patches;pid1++){
					for (int pid2=0; pid2<N_Particle[id1]->N_patches;pid2++){
						...
						particle_dist_y = N_Particle[id1]->y_patch[pid1] - N_Particle[id2]->y_patch[pid2];
						particle_dist_z = N_Particle[id1]->z_patch[pid1] - N_Particle[id2]->z_patch[pid2];
						...
						if ( patch_distance_squared < N_Particle[id1]->patch_cutoff_squared[pid1] && id1!= id2 && N_Particle[id1]->patch_energy[type_1][type_2]!=0){
