...
#include "pmove.h"
	...
	pmove::pmove(int size, int edge_length, double delta_tmax, double delta_rmax, double delta_Vmax, double Temperature, string is_translation_ON_in, string is_rotation_ON_in, string is_volumemove_ON_in, string is_grand_canonical_ON_in, string is_cluster_move_ON_in, int is_2D_in){
	...
	void pmove::Trans_Update_Positions(particles& Particles, int id, m_vector &trans_vec){
	...
	void pmove::Rot_Update_Positions(particles& Particles, int id, double (&Rot_mat)[9]){
	...
	void move::Rot_Move_Map(particles& Particles, int id1, box* Box, m_vector &center_mass, double (&Rot_mat)[9]){
		...
		map_dist_center.z = particle_dist_z;
	   	...
	   	map_rot_center.x  = Rot_mat[0]*map_dist_center.x + Rot_mat[1]*map_dist_center.y + Rot_mat[2]*map_dist_center.z ;
		map_rot_center.y  = Rot_mat[3]*map_dist_center.x + Rot_mat[4]*map_dist_center.y + Rot_mat[5]*map_dist_center.z ;
		map_rot_center.z  = Rot_mat[6]*map_dist_center.x + Rot_mat[7]*map_dist_center.y + Rot_mat[8]*map_dist_center.z ;
		...
		for(int j=0;j<Particles.N_Particle[id1]->edge_N;j++){
			...
			Particles.N_Particle[id1]->new_dist_x[j] = Rot_mat[0]*map_dist[j].x + Rot_mat[1]*map_dist[j].y + Rot_mat[2]*map_dist[j].z ;
			Particles.N_Particle[id1]->new_dist_y[j] = Rot_mat[3]*map_dist[j].x + Rot_mat[4]*map_dist[j].y + Rot_mat[5]*map_dist[j].z ;
			Particles.N_Particle[id1]->new_dist_z[j] = Rot_mat[6]*map_dist[j].x + Rot_mat[7]*map_dist[j].y + Rot_mat[8]*map_dist[j].z ;
				
				
			Particles.N_Particle[id1]->x[j] = center_mass.x + Particles.N_Particle[id1]->new_dist_x[j]; 
			Particles.N_Particle[id1]->y[j] = center_mass.y + Particles.N_Particle[id1]->new_dist_y[j];
			Particles.N_Particle[id1]->z[j] = center_mass.z + Particles.N_Particle[id1]->new_dist_z[j]; 
	...
	void move::Set_Positions(particles& Particles, int id){
		...
		Particles.N_Particle[id]->q.z = 1.0*sin(Particles.N_Particle[id]->phi/2.0); 
