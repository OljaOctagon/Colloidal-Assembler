...
pmove();
pmove(int size, int edge_length, double delta_tmax, double delta_rmax, double delta_Vmax, double Temperature, string is_translation_ON, string is_rotation_ON, string is_volumemove_ON, string is_grand_canonical_ON, string is_cluster_move_ON, int is_2D);
~pmove();
...
void Collision_Test_Halo( particles& Particles, box* Box, int id, collision_list* Collision_List);
...
void Rotate2D( particles& Particles, box* Box, fileio& Fileio, int id, int mc_time);
...
double Calculate_Pair_Potential( int id, particles& Particles, box* Box, collision_list* Collision_List);
void Calculate_Pair_Potential(particles& Particles, box* Box, collision_list* Collision_List);
void Calculate_Pair_Potential( particles& Particles, box* Box);
double Calculate_Patch_Distance(int id1, int id2, int pid1, int pid2, particles& Particles, box* Box);
void Calculate_Pair_Potential_Neighbours( particles& Particles, box* Box, int id1);

void Particle_Insertion(particles& Particles, box* Box, fileio& Fileio, int mc_time);
void Particle_Deletion(particles& Particles, box* Box, fileio& Fileio, int mc_time);

double Calculate_Potential(int a, int b, particles& Particles, box* Box);
void Reset_Pseudo_Cluster(box* Box);
void Trans_Pseudocluster_Recursion(int id_j, int cn, int fl, particles& Particles, box* Box);
void Trans_Cluster_Move(particles& Particles, box* Box, fileio& Fileio, int mc_time);

void Pseudocluster_Recursion(int id_j, int cn, int fl, particles& Particles, box* Box);
void Rot_Cluster_Move(particles& Particles, box* Box, fileio& Fileio, int mc_time);
void Check_Periodic_Center_of_Mass(m_vector &center_mass, box* Box);

void Three_Particle_Move(particles& Particles, box* Box, fileio& Fileio, int mc_time);
void Rot_Move_Map(particles& Particles, int id1, int id2, box* Box, double (&Rot_mat)[9]);
void Rot_Move_Map(particles& Particles, int id1, box* Box, m_vector &center_mass, double (&Rot_mat)[9]);
