#include "move.h"
    ...
    void move::Translate(particles& Particles, box* Box, fileio& Fileio, int id, int mc_time){
		...
		/*
		Particles.N_Particle[id]->x_center = Particles.N_Particle[id]->x_center - Box->Lx/2.;
		Particles.N_Particle[id]->y_center = Particles.N_Particle[id]->y_center - Box->Lx/2.;
		
		Particles.N_Particle[id]->x_center = Particles.N_Particle[id]->x_center - Box->Lx*rint(Particles.N_Particle[id]->x_center/Box->Lx);
		Particles.N_Particle[id]->y_center = Particles.N_Particle[id]->y_center - Box->Lx*rint(Particles.N_Particle[id]->y_center/Box->Lx);
			    
		Particles.N_Particle[id]->x_center = Particles.N_Particle[id]->x_center + Box->Lx/2.;
		Particles.N_Particle[id]->y_center = Particles.N_Particle[id]->y_center + Box->Lx/2.;
		*/

		// be careful only sets gears to right orientation! Need to fix the other particles as well. To do other particles
		// uncomment the part in Trans_Update_Positions and Particles.N_Particle[id]->edges_from_center()!
