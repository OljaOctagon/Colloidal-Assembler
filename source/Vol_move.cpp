...
void pmove::Iso_Vol_Change(particles &Particles, box *Box, fileio &Fileio,
    ...
    if (b_factor >= XI) {
        ...
        for (int id = 0; id < Box->N; id++) {
            ...
            trans_vec.x =
                Box->x_center +
                (Particles.N_Particle[id]->x_center - Box->x_center) *
                    Box->Lx_scale -
                Particles.N_Particle[id]->x_center;
            trans_vec.y =
                Box->y_center +
                (Particles.N_Particle[id]->y_center - Box->y_center) *
                    Box->Ly_scale -
                Particles.N_Particle[id]->y_center;
            // trans_vec.z = Box->z_center +
            // (Particles.N_Particle[id]->z_center -
            // Box->z_center)*Box->Lz_scale -
            // Particles.N_Particle[id]->z_center;
