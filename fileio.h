...
void Write_Cell_Positions(int number_of_cells, cell* Cell, int mc_time, box* Box);
void Write_Order_Parameters(int mc_time, double D2_order, double D4_order);
void Write_Vol_Packing(int mc_time, box* Box);
void Write_Local_Order_Parameters(int mc_time, particles& Particles, box* Box, order_parameter& Order_Parameter, cluster& Cluster);
void Write_Cluster_List(int* Cluster_List, int cluster_size, particles& Particles, order_parameter& Order_Parameter);			 
void Write_Box_Positions( int mc_time, box* Box);
void Write_g_r(int mc_time, order_parameter& Order_Parameter);

void Save_Config(box* Box, particles& Particles, gsl_rng *r, gsl_rng *r01, double dmax_t, double dmax_alpha, double dmax_beta, double dmax_gamma, double dmax_V, double dmax_L, int mc_time);
...
void Save_Orientations_Axis_Angle(box* Box, particles& Particles, int mc_time);
void Save_Random_State(gsl_rng *r, gsl_rng *r01, int mc_time);

void Read_Config(box* Box, particles& Particles, gsl_rng *r, gsl_rng *r01, int mc_time);
...
void Read_Orientations_Axis_Angle(box* Box, particles& Particles, int mc_time);
void Read_Box(box* Box, int mc_time);
void Read_Random_State(gsl_rng *r, gsl_rng *r01, int mc_time);

void Write_Acceptances(int mc_time, double accept_translate_procent, double accept_rotate_procent, double accept_iso_vol_procent, double accept_complete_procent);
