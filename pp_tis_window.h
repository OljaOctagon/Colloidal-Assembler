...
using namespace std;
	...
	tis_window(int wv_0_in, int wv_i_in, int wv_ip1_in, int term_time, char traj_type_in[128]);
	void Get_path_info(int wv, int mc_time);
	void Get_path_info_BACKWARDS(int wv, int mc_time);
	
	void Write_path_info(int N_check_points, int check_point_frequency, int mc_time);
	bool tis_controller(int wv, int mc_time, int N_check_points, int check_point_frequency);
