...
#mu,energy,delta,topology,packing_fraction,n_liquid,n_micell_1,n_micell_2,n_chain_and_loop
...
for ei, nei in zip(Epsi[::-1], range(len(Epsi))):
	for pi, npi in zip(Pos, range(len(Pos))):
		...
		# bigger than 5 
		e =  df_sub.n_chain_and_loop.values[0] + df_sub.n_micell_1.values[0]
		
		# smaller than 6 
		f =  df_sub.n_liquid.values[0] + df_sub.n_micell_2.values[0]
...
x_og = np.arange(0,7,1)
y_og = np.arange(0,6,1)
...
ylabels=[-10.2,-9.2,-8.2,-7.2,-6.2,-5.2]
