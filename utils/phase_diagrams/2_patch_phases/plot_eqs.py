...
X_03 = pd.read_csv("eqs_0.3.dat", header=None, delim_whitespace=True).values
X_05 = pd.read_csv("eqs_0.5.dat", header=None, delim_whitespace=True).values
X_07 = pd.read_csv("eqs_0.7.dat", header=None, delim_whitespace=True).values
...
plt.errorbar(X_03[:,1], X_03[:,0], xerr=X_03[:,2],
             lw=0, ms=8, marker='o', c='#ff5e62', alpha=0.7)

plt.errorbar(X_05[:,1], X_05[:,0], xerr=X_05[:,2],
             lw=0, ms=8, marker='s', c='#00f2a3', alpha=0.7)

plt.errorbar(X_07[:,1], X_07[:,0], xerr=X_07[:,2],
             lw=0, ms=8, marker='^', c='#6300ff', alpha=0.7)
...
plt.tight_layout()
plt.savefig("eqs_boxes.png", dpi=300)
