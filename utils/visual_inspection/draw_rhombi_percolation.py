...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-ptype', type=str, choices=['dma-as2', 'feq-as2', 'dma-as1', 'dmo-s1', 'dmo-s2', 'dmo-as1'])
    ...
    colors_orient = cmr.take_cmap_colors(
        'cmr.infinity_s', 180, cmap_range=(0, 1), return_fmt='hex')
    ...
    for j, val in enumerate(check_point_values[-100:]):
        ...
        for i in range(N):
            ...
            oi = orient_i[i] - np.floor(orient_i[i]/np.pi)*np.pi
            rhombus_color = colors_orient[int(180*oi/np.pi)-1]
            ...
            rhombi = patches.Polygon(
                edges, linewidth=0.5, edgecolor='k', facecolor=rhombus_color, alpha=0.7)
            ax.add_patch(rhombi)
