...
def area_rhombi(Lx,patch_pos):
    ...
    pore_p = np.fabs(2*patch_pos - 1)
    pore_size = pore_p*Lx
    pore_area = np.power(pore_size,2)*np.sin(np.pi/3.)
    return pore_area
...
fig, ax = plt.subplots()
ax.plot(patch_pos,area_h, lw=2, marker='h', c='#336699', 
    label='$\displaystyle \\textrm{hexagonal pore}$',
        alpha=1.0)

ax.plot(patch_pos,area_t, lw=2, marker='^', c='#000080', 
...
ax.legend(loc='upper center', fancybox=True, prop={'size': 21}, framealpha=1.0)
ax.tick_params(axis='both', which='major', labelsize=22)
ax.set_xlabel("$ \displaystyle \Delta$", size=30)
ax.set_ylabel("$ \displaystyle \\textrm{pore area} [1/l^2]$", size=30)
plt.tight_layout()
