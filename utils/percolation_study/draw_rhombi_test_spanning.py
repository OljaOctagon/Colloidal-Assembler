...
if __name__ == '__main__':
    ...
    pos_i = np.load("virtual_pos.npy")
    orient_i = np.load("virtual_orient.npy")
    box_l = np.load("virtual_box.npy")

    N=len(pos_i)
    assert (N == 1500*9), "Wrong number of particles" 
    ...
    bx=box_l[0]
    by=box_l[1]
    plt.vlines(np.array([-bx/3,0,bx/3,2*bx/3]), -bx/2+bx/6, bx/2+bx/6,lw=0.1)
