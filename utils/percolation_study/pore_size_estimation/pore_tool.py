...
class Rhombi:
    def __init__(self, pos, orient, lx, ndim):
        ...
        for i in range(self.Np):
            ...
            self.vertices[i, 0] = get_edge_points(
                p, self.ax_n[i], np.array([-1, -1]))
            self.vertices[i, 1] = get_edge_points(
                p, self.ax_n[i], np.array([+1, -1]))
            self.vertices[i, 2] = get_edge_points(
                p, self.ax_n[i], np.array([+1, +1]))
            self.vertices[i, 3] = get_edge_points(
                p, self.ax_n[i], np.array([-1, +1]))
