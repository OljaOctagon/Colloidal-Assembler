...
void cluster::HarvestClusterRecursion(int cluster_number, int id_j,
                                      particles &Particles) {
    ...
    for (int j = 0; j < Particles.Collision_List[id_j].Nm; j++) {
        ...
        if (Particles.Collision_List[id_j].Elements[j].is_neighbour == 1) {
            ...
            if (op_cluster_info[list_j].isactive == true &&
                op_cluster_info[list_j].cluster_id == -1) {
...
void cluster::op_cluster_build(box *Box, particles &Particles) {
    ...
    for (int id = 0; id < Box->N; id++) {
        if ((op_cluster_info[id].isactive == true) &&
            (op_cluster_info[id].cluster_id == -1)) {
            cluster_number++;
...
int cluster::op_cluster_get_largest_cluster_size(int cnums, box *Box,
    ...
    // cout<<"op_cluster_info[id].cluster_id:
    // "<<op_cluster_info[4].cluster_id<<endl;

    for (int id = 0; id < Box->N; id++) {
        if (op_cluster_info[id].isactive) {

            l = l + 1;
            a[op_cluster_info[id].cluster_id - 1] =
                a[op_cluster_info[id].cluster_id - 1] + 1;
        }
    }
        ...
        for (int j = 0; j < cnums; j++) {
            ...
            if (a[j] == 3) {
                ...
                for (int id = 0; id < Box->N; id++) {
                    if ((op_cluster_info[id].cluster_id - 1) == j) {
                        theta_x =
                            (Particles.N_Particle[id]->x_center / Box->Lx) *
                            2 * M_PI;
                        theta_y =
                            (Particles.N_Particle[id]->y_center / Box->Ly) *
                            2 * M_PI;
                ...
                r1_out << time_s << " " << cnums2 << " " << Box->Lx << " "
                       << Box->Ly << " " << k << " " << center_mass.x << " "
                       << center_mass.y << endl;
...
void cluster::Calculate(order_parameter &Order_Parameter, particles &Particles,
                        box *Box, int time_s) {
    ...
    for (int id = 0; id < Box->N; id++) {
        if (Order_Parameter.is_nPhase[id] ==
            1.0) { // Kriterum für Cubus fest/flüssig
...
void cluster::Calculate(order_parameter &Order_Parameter, particles &Particles,
    ...
    Size_R = op_cluster_get_largest_cluster_size(cnums, Box, Particles, time_s,
                                                 is_mass_calc);
...
/*
   ...
   .) ClusterListe init und setzen
    ...
    if(calc_BCtWm == 1) {
      ...
      .)  op_cluster_build();
        //BCtWm = double(op_cluster_get_largest_cluster_size()); // Groesse des
   Grossten Clusters BCtWm = op_cluster_get_largest_cluster_size_weighted();
        for(int i=0; i<natom; i++) {
