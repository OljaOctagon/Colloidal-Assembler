...
void pmove::change_to_fractional(particles &Particles, box *Box) {
    ...
    */
    ...
    for (id = 0; id < Box->N; id++) {
        ...
        Particles.N_Particle[id].x_center_scaled =
            M[0] * Particles.N_Particles[id].x_center +
            M[1] * Particles.N_Particles[id].y_center +
            M[2] * Particles.N_Particles[id].z_center;
        Particles.N_Particle[id].y_center_scaled =
            M[3] * Particles.N_Particles[id].x_center +
            M[4] * Particles.N_Particles[id].y_center +
            M[5] * Particles.N_Particles[id].z_center;
        Particles.N_Particle[id].z_center_scaled =
            M[6] * Particles.N_Particles[id].x_center +
            M[7] * Particles.N_Particles[id].y_center +
            M[8] * Particles.N_Particles[id].z_center;
    }
}
    ...
    */
    ...
    for (id = 0; id < Box->N; id++) {
        ...
        Particles.N_Particle[id].x_center =
            M[0] * Particles.N_Particles[id].x_center_scaled +
            M[1] * Particles.N_Particles[id].y_center_scaled +
            M[2] * Particles.N_Particles[id].z_center_scaled;
        Particles.N_Particle[id].y_center =
            M[3] * Particles.N_Particles[id].x_center_scaled +
            M[4] * Particles.N_Particles[id].y_center_scaled +
            M[5] * Particles.N_Particles[id].z_center_scaled;
        Particles.N_Particle[id].z_center =
            M[6] * Particles.N_Particles[id].x_center_scaled +
            M[7] * Particles.N_Particles[id].y_center_scaled +
            M[8] * Particles.N_Particles[id].z_center_scaled;
    }
}

void pmove::Vol_deform(particles &Particles, box *Box, fileio &Fileio,
                       int mc_time) {
    ...
    // CHANGE BOX: ADD A SMALL PERTURBATION TO ONE OF THE COORDINATES OF ONE OF
    // THE VECTORS
    ...
    b_factor_pre = exp(-1.0 * Box->P_sigma * (Box->V - Box->V_old) +
                       Box->N * log(Box->V_rel));
    ...
    if (b_factor < XI) {
        ...
        do {
            ...
            Particles.Collision_List[id].Calculate(
                Box, id, Particles.Id_Cell_List, Particles.Cell_List,
                Particles.Cell, Particles.N_Particle,
        ...
        if (exit_status >= 1) {
            ...
            for (int c_id = 0; c_id < Particles.number_of_cells; c_id++) {
                ...
                Particles.Cell[c_id].x_center =
                    Box->x[0] + Particles.Cell[0].Lx / 2.0 +
                    Particles.Cell[0].Lx * double(c_id % Particles.N_c);
                Particles.Cell[c_id].y_center =
                    Box->y[0] + Particles.Cell[0].Ly / 2.0 +
                    Particles.Cell[0].Ly *
                        double((c_id / Particles.N_c) % Particles.N_c);
                Particles.Cell[c_id].z_center =
                    Box->z[0] + Particles.Cell[0].Lz / 2.0 +
                    Particles.Cell[0].Lz *
                        double(c_id / (Particles.N_c * Particles.N_c));
