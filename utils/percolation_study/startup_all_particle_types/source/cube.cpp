...
void cube::write_positions(ofstream &fout) {}

void cube::relative_distances() {
...
void cube::edges_from_center() { // only valid if cubes are aligned with
                                 // coordinate axes!
...
void cube::Set_Lengths(double a) {
...
void cube::Set_Start_Lattice_Position(int id, double box_Lx, double box_Ly,
    ...
    x_center = 0.5 + l_distpx / 2.0 +
               double(id % N_sitespx + l_distpx * (id % N_sitespx));
    y_center = 0.5 + l_distpy / 2.0 +
