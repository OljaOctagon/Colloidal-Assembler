...
void gear::get_relative_orientation() {
    ...
    for (int i = 0; i < Ng; i++) {
        ...
        for (int k = 0; k < gear_wheels[0].edge_N; k++) {
            ...
            d_pos.x = Rot_mat[0] * gear_wheels[i].rel_pos[k].x +
                      Rot_mat[1] * gear_wheels[i].rel_pos[k].y +
                      Rot_mat[2] * gear_wheels[i].rel_pos[k].z;
            d_pos.y = Rot_mat[3] * gear_wheels[i].rel_pos[k].x +
                      Rot_mat[4] * gear_wheels[i].rel_pos[k].y +
                      Rot_mat[5] * gear_wheels[i].rel_pos[k].z;
            d_pos.z = Rot_mat[6] * gear_wheels[i].rel_pos[k].x +
                      Rot_mat[7] * gear_wheels[i].rel_pos[k].y +
                      Rot_mat[8] * gear_wheels[i].rel_pos[k].z;
...
void gear::edges_from_center() {
    ...
    for (int i = 0; i < Ng; i++) {
        ...
        for (int k = 0; k < gear_wheels[i].edge_N; k++) {
            ...
            gear_wheels[i].x[k] =
                gear_wheels[i].x_center + gear_wheels[i].rel_pos[k].x;
            gear_wheels[i].y[k] =
                gear_wheels[i].y_center + gear_wheels[i].rel_pos[k].y;
            gear_wheels[i].z[k] =
                gear_wheels[i].z_center + gear_wheels[i].rel_pos[k].z;
        }
    }
}

void gear::Set_Start_Lattice_Position(int id, double box_Lx, double box_Ly,
    ...
    x_center = R / 2.0 + l_distpx / 2.0 +
               double(id % N_sitespx + l_distpx * (id % N_sitespx));
    y_center = R / 2.0 + l_distpy / 2.0 +
               double((id / N_sitespx) % N_sitespy +
                      l_distpx * ((id / N_sitespy) % N_sitespx));
    z_center = Lx / 2.0;
    // z_center = 0.5 + l_distpz/2.0 + double(id/int(pow((double)N_sitespx,2))
    // + l_distpz*(id/int((double)pow(N_sitespx,2))));
...
void gear::write_positions(ofstream &fout) {
    for (int i = 0; i < Ng; i++) {
        for (int k = 0; k < gear_wheels[i].edge_N; k++) {
            fout << "zr"
                 << "      " << gear_wheels[i].x[k] << "   "
                 << gear_wheels[i].y[k] << "   " << gear_wheels[i].z[k]
                 << endl;
        }
    }
}
