...
static void build_fcc(void) {
    ...
    if (gsl_fcmp((double)cells, test, 1.0e-12)) {
        fprintf(stderr, "Wrong number of molecules. %d instead of %d or %d\n",
                nmol, 4 * cells * cells * cells,
                4 * (cells + 1) * (cells + 1) * (cells + 1));
        exit(EXIT_FAILURE);
    }
    a = cbrt(4.0 / rho);
...
static void build_hcp(void) {
    ...
    double s[3];
    double b1[3] = {1.0, 0.0,
                    0.0}; // first basis vector (with unit lattice constant)
    ...
    for (i = 0; i < cells[0]; i++)
        for (j = 0; j < cells[1]; j++)
            for (k = 0; k < cells[2]; k++) {
                ...
                r[z][0] = s[0] + a1[0] * a * b1[0] + a1[1] * a * b2[0] +
                          a1[2] * a * b3[0];
                r[z][1] = s[1] + a1[0] * a * b1[1] + a1[1] * a * b2[1] +
                          a1[2] * a * b3[1];
                r[z][2] = s[2] + a1[0] * a * b1[2] + a1[1] * a * b2[2] +
                          a1[2] * a * b3[2];
                z++;

                r[z][0] = s[0] + a2[0] * a * b1[0] + a2[1] * a * b2[0] +
                          a2[2] * a * b3[0];
                r[z][1] = s[1] + a2[0] * a * b1[1] + a2[1] * a * b2[1] +
                          a2[2] * a * b3[1];
                r[z][2] = s[2] + a2[0] * a * b1[2] + a2[1] * a * b2[2] +
                          a2[2] * a * b3[2];
                z++;
            }
...
static void build_bcc(void) {
    ...
    if (gsl_fcmp((double)cells, test, 1.0e-12)) {
        fprintf(stderr, "Wrong number of molecules. %d instead of %d or %d\n",
                nmol, 2 * cells * cells * cells,
                2 * (cells + 1) * (cells + 1) * (cells + 1));
        exit(EXIT_FAILURE);
    }
    a = cbrt(2.0 / rho);
    fprintf(stderr, "a=%lf\n", a);
