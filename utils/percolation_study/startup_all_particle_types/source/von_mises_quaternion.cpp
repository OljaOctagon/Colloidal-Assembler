...
void pmove::Rot_Update_Quarternions_VON_MISES(particles &Particles, int id) {
    ...
    do {
        ...
        u = gsl_rng_uniform(r01) w = (1 - (1 + b) * z) / (1 - (1 - b) * z);
    ...
    q_t.w = w;
    q_t.x = sp_x * sqrt(1 - w * w) q_t.y =
                sp_y * sqrt(1 - w * w) q_t.z = sp_zsqrt(1 - w * w)

                                                   q_s.w =
                    q_t.w * mu.w - q_t.x * mu.x - q_t.y * mu.y - q_t.z * mu.z;
    q_s.x = q_t.w *mu.x + q_t.x *mu.w + q_t.y *mu.z - q_t.z *mu.x q_s.y =
                q_t.w *mu.y - q_t.x *mu.z + q_t.y *mu.w + q_t.z *mu.x q_s.z =
                    q_t.w * mu.z + q_t.x * mu.y - q_t.y * mu.x + q_t.x * mu.w;
    ...
    orient_out << Particles.N_Particle[id].q.w << "  "
               << Particles.N_Particle[id].q.x << "  "
               << Particles.N_Particle[id].q.y << "  "
               << Particles.N_Particle[id].q.z << endl;
    orient_out.close();
}
